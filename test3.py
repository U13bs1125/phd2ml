...
df4 = pd.read_csv("data/weather_soil_agro_with_ari.csv")
# a_
ari_crop_cols = [c for c in df3.columns if c.startswith("ARI_")]
crop_cols = [c for c in df4.columns if c.startswith("Crop_")]
df4 = pd.concat([df4[crop_cols], df3[ari_crop_cols]], axis=1)
...
df = pd.concat([df, df4.loc[:, ~df4.columns.isin(df.columns)]], axis=1)
trg = ["Aflac", "Fumc", "Afla", "Fum"]
# remove trg if exists from df
...
for target in targets:
    ...
    for country in countries:
        ...
        df_train = df[df["Country"] != country]
        ...
        if df_test.empty or df_train.empty:
            ...
            # ---------------- FEATURES ----------------
        if target == 'Aflac':
            X_train = df_train.drop(columns=['Aflac', "Country", "Afla_raw"])
        else:
            X_train = df_train.drop(columns=['Fumc', "Country", "Fum_raw"])
        y_train = df_train[target]

        if target == 'Aflac':
            X_test = df_test.drop(columns=['Aflac', "Country", "Afla_raw"])
        else:
            X_test = df_test.drop(columns=['Fumc', "Country", "Fum_raw"])
...
results_df.to_csv("results/test3_results.csv", index=False)

print("\n✅ Results saved to results/test3_results.csv")
